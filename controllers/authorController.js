...
exports.author_update_get = function(req, res, next) {
    ...
    Author.findById(req.params.id, function (err, results) { //results variable is the same across the other two positions
            if (err) { return next(err); }
            if (results==null) { // No results. (here)
                var err = new Error('Author not found');
                err.status = 404;
                return next(err);
            }
            // Success.
            res.render('author_form', { title: 'Update Author', author: results }); //(and here)
        });
...
exports.author_update_post = [
    ...
    sanitizeBody('first_name').trim().escape(),
    sanitizeBody('family_name').trim().escape(),
    sanitizeBody('date_of_brith').trim().escape(),
